...
// Reserve heroes passively train at 15% of active squad XP rate.
// Level cap: reserve hero can only catch up to the lowest active squad member's level.
const ReserveTrainingManager = {
  ...
  getXpRate(campaignProgress) {
    ...
    return this.BASE_XP_RATE * (1 + regions * 0.3) * ElderTreeManager.getReserveTrainingXpMult();
  },
  ...
  _distributeXp(xpPerHero, activeSquad) {
    ...
    for (const hero of this.getBenchedHeroes(activeSquad)) {
      ...
      // Cap XP at current threshold so the bar does not silently overflow.
...
export default ReserveTrainingManager;
